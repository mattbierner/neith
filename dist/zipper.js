 ...
 * DO NOT EDIT
*/
define(["require", "exports", "nu/stream", "nu/select"], (function(require, exports, __o, __o0) {
    "use strict";
    var path, lefts, rights, children, parent, hasChildren, hasParent, isRoot, isChild, isLeaf, isFirst, isLast, up, down, left, right, root, leftmost, rightmost, leftLeaf, rightLeaf, nextUpDfs, nextDfs, prevDfs, extract, replace, modify, remove, setLefts, modifyLefts, setRights, modifyRights, insertLeft, insertRight, insertChild, appendChild, zipper, treeZipper;
    var __o = __o,
        append = __o["append"],
        cons = __o["cons"],
        first = __o["first"],
    ...
    var constructParent = (function(ctx) {
        return construct(ctx, parent(ctx).focus, append(reverse(lefts(ctx)), cons(extract(ctx), NIL), rights(ctx)));
    });
    ...
    (down = (function(ctx) {
        return (isLeaf(ctx) ? null : setLoc(ctx, (function() {
            {
                var cs = children(ctx);
                return new(Loc)(first(cs), getLoc(ctx), pushPath(extract(ctx), ctx), NIL, rest(cs));
            }
        })()));
    }));
    (left = (function(ctx) {
        return (isFirst(ctx) ? null : setLoc(ctx, (function() {
            {
                var ls = lefts(ctx);
                return Loc.setSurround(getLoc(ctx), rest(ls), first(ls), cons(extract(ctx), rights(ctx)));
            }
        })()));
    }));
    (right = (function(ctx) {
        return (isLast(ctx) ? null : setLoc(ctx, (function() {
            {
                var rs = rights(ctx);
                return Loc.setSurround(getLoc(ctx), cons(extract(ctx), lefts(ctx)), first(rs), rest(rs));
            }
        })()));
    }));
    (root = (function(ctx) {
    ...
    (remove = (function(ctx) {
        return (isLast(ctx) ? (isFirst(ctx) ? (hasParent(ctx) ? constructParent(ctx) : null) : setRights(NIL, left(ctx))) : modifyLefts(skip.bind(null, 2), right(ctx)));
    }));
    ...
    (insertChild = (function(node, ctx) {
        return (hasChildren(ctx) ? up(insertLeft.bind(null, node)(down(ctx))) : replace(construct(ctx, extract(ctx), cons(node, NIL)), ctx));
    }));
    (appendChild = (function(node, ctx) {
        return (hasChildren(ctx) ? up(insertRight.bind(null, node)(rightmost(down(ctx)))) : insertChild(node, ctx));
    }));
