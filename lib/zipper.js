...
define(["require", "exports"], (function(require, exports) {
    "use strict";
    var Loc, Context, path, lefts, rights, getChildren, getChild, getParent, hasChildren, hasParent, isRoot, isChild, isLeaf, isFirst, isLast, up, down, left, right, root, leftmost, rightmost, leftLeaf, rightLeaf, nextUpDfs, nextDfs, prevDfs, child, nthChild, sibling, getNode, setNode, modifyNode, removeNode, insertLeft, insertRight, insertChild, appendChild, zipper, treeZipper;
    ...
    var setPath = (function(x, ctx) {
        return concat(x, slice(path(ctx, 1)));
    });
            ...
            var m = (function(x) {
                ...
                reducer = (function(p, c) {
                    ...
                    return (function(ctx, parent, children) {
                        return ctx.constructNode(parent, map(children, m), reduce(children, reducer, ({})));
                    });
            }
        }
    })();
        ...
        return (!hasChildren(ctx) ? null : (function() {
            {
                var children = getChildren(ctx),
                    child = children[0]; {
                        return setLoc(ctx, new(Loc)(getChild(ctx, child), getLoc(ctx), pushPath(child, ctx), [], slice(children, 1).map((function(x) {
        ...
        return (function() {
            {
                var ls = lefts(ctx),
                    l = ls[0]; {
                        return (!ls.length ? null : setLoc(ctx, new(Loc)(l.value, getParent(ctx), setPath(l.key, ctx), slice(ctx.loc.left, 1), concat(Pair(path(ctx)[0], getFocus(ctx)), ctx.loc.right))));
                }
            }
        })();
    }));
        ...
        return (function() {
            {
                var rs = rights(ctx),
                    r = rs[0]; {
                        return (!rs.length ? null : setLoc(ctx, new(Loc)(r.value, getParent(ctx), setPath(r.key, ctx), concat(ctx.loc.left, Pair(path(ctx)[0], getFocus(ctx))), slice(ctx.loc.right, 1))));
                }
            }
        })();
    }));
    ...
    (removeNode = (function(ctx) {
        return (!hasParent(ctx) ? null : (isLast(ctx) ? (isFirst(ctx) ? construct(ctx, ctx.loc.parent, []) : setRights(left(ctx), [])) : modifyLefts(right(ctx), (function(lefts) {
    ...
    (appendChild = (function(edge, node, ctx) {
        return (hasChildren(ctx) ? up(insertRight.bind(null, edge, node)(rightmost(down(ctx)))) : setNode(construct(ctx, ctx.loc.focus, [Pair(edge, node)]), ctx));
    }));
