...
(down = (function(ctx) {
    ...
    return (isEmpty(cs) ? null : ((loc = new(Loc)(first(cs), ctx.loc, pushPath(extract(ctx), ctx), NIL, rest(cs),
        false)), ctx.setLoc(loc)));
}));
(left = (function(ctx) {
    var ls, loc;
    return (isFirst(ctx) ? null : ((ls = lefts(ctx)), (loc = ctx.loc.setSurround(rest(ls), first(ls), cons(
        extract(ctx), rights(ctx)))), ctx.setLoc(loc)));
}));
(right = (function(ctx) {
    var rs, loc;
    return (isLast(ctx) ? null : ((rs = rights(ctx)), (loc = ctx.loc.setSurround(cons(extract(ctx), lefts(ctx)),
        first(rs), rest(rs))), ctx.setLoc(loc)));
}));
